...
IFS=$'\t' read -r current_dir model_name cost lines_added lines_removed duration_ms ctx_used cache_pct five_h_resets five_h_pct seven_d_resets seven_d_pct < <(
...
# Rate limit chips (Pro/Max only; absent until first API response of session)
now_epoch=$(date +%s)
...
build_limit_chip() {
    local resets=$1 pct=$2
    [ "$resets" -gt 0 ] 2>/dev/null || return
    local remaining=$((resets - now_epoch))
    local color
    if [ "$pct" -lt 50 ]; then color='\033[32m'
    elif [ "$pct" -lt 80 ]; then color='\033[33m'
    else color='\033[31m'
    fi
    printf '%b⟳%s\033[0m' "$color" "$(format_remaining "$remaining")"
}

chip_5h=$(build_limit_chip "$five_h_resets" "$five_h_pct")
chip_7d=$(build_limit_chip "$seven_d_resets" "$seven_d_pct")
...
if [ -n "$chip_5h" ]; then
    line2="$line2 $(printf '%b ' "$SEP")$chip_5h"
fi
if [ -n "$chip_7d" ]; then
    line2="$line2 $chip_7d"
fi
